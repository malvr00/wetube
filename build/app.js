...
app.use(function (req, res, next) {
  res.setHeader("Content-Security-Policy", "script-src 'unsafe-eval' 'self' https://archive.org");
  return next();
});
...
app.use((0, _expressSession["default"])({
  ...
  resave: false,
  saveUninitialized: false // store: MongoStore.create({ mongoUrl: process.env.MONGO_URL })

})); //passport가 스스로 쿠키를 찾아 쿠키에 해당하는 사용자 찾음

app.use(_passport["default"].initialize()); // passport 초기화

app.use(_passport["default"].session()); // 자기가 찾은 사용자를 요청(req)로 만들어줌

app.use(_localsmiddleware.localsmiddleware);
...
app.use(_routes["default"].api, _apiRouter["default"]); //app.use("/user", userRouter);
